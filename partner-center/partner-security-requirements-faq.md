...
藉由針對所有使用者帳戶強制執行多重要素驗證，任何想要以非互動方式執行的自動化或整合都會受到影響。 儘管合作夥伴安全性需求需要您針對合作夥伴中心 API 啟用安全應用程式模型，但您可以利用它來解決自動化和整合進行第二個要素驗證的需求。 請注意，存取的資源必須支援以存取權杖為基礎的驗證。
