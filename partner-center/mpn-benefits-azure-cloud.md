...
- 無論授權型訂用帳戶或以使用量為基礎的訂用帳戶，您都可以使用各種 Azure & 雲端產品訂用帳戶權益
- 為每個產品指派的授權數量
- 布建特定權益產品給您的供應專案類型
- 權益類型 (是否來自核心或增量) 
- 特定權益到期的日期
...
- **每月的 Azure 點數布建點數：** 這可能會顯示為像是 $ 100/month (或當地貨幣的相等) <br/>
*and/or*
...
1. 在 [Azure 與雲端產品] 頁面上找出特定訂用帳戶。 

1. 選取右邊的下拉箭號，然後查看該訂用帳戶的 [ **詳細資料] 資料** 行。 

1. 閱讀後續頁面上的指示。 這麼做會讓您更深入瞭解如何兌換金鑰。 

   :::image type="content" source="images/benefits/mpn-benefits-azure-cloud-3.png" alt-text="具有查看詳細資料的 Azure 和雲端產品頁面。" border="false":::


1. 當您準備好兌換金鑰時，請選取此頁面底部的 **啟用逐步** 解說選項。
   - 如果您想要啟用新的訂用帳戶，或現有的訂用帳戶已停用/過期，請開啟 **私人或 incognito 瀏覽器**，然後複製並貼上下列連結，並遵循指示進行：

      [https://signup.microsoft.com/productkeystart](https://signup.microsoft.com/productkeystart)
      
   - 如果您想要延長結束日期，或將更多基座/授權新增至現有的訂用帳戶複本，並在 **私用或 incognito 瀏覽器視窗** 中貼上下列連結，並遵循指示進行：
 
      [https://admin.microsoft.com/AdminPortal/Home#/subscriptions](https://admin.microsoft.com/AdminPortal/Home#/subscriptions)

> [!Note]
> 一旦兌換金鑰，就無法再次兌換相同的金鑰。 另請注意下列重點：
>
> - 確認您是否需要新增更多授權 (基座) 、數量，以及是否要延長結束日期。 如果您選擇新增更多授權 (基座) 而且您 **未** 延長結束日期，系統會提供您少於12個月的有效訂用帳戶。 
>
...
請遵循下列步驟來新增更多授權，或延長現有訂用帳戶的結束日期：

1. 複製下列連結，並將其貼入 **私用或 incognito 瀏覽器視窗**。
...
1. 登入您的 **M365 admin center 入口網站** 時，請確定您是使用正確的 Azure Active Directory 租使用者登入。 然後，從清單中選取特定的雲端訂用帳戶，然後執行下列其中一個 (以更新訂用帳戶或新增更多授權) ：
   ...
   - 若要新增更多授權 (基座) ）至現有的訂用帳戶，請在 [授權] 區域中選取 [ **新增授權** ] 選項。 如果您也想要更新現有的訂用帳戶，請確定您未選取 [ **新增授權** ]。 選取 [ **新增授權** ] 將會縮短訂用帳戶的持續時間，並增加訂用帳戶中的授權數目。

   :::image type="content" source="images/benefits/mpn-benefits-azure-cloud-4.png" alt-text="顯示如何新增授權的 Azure 和雲端產品頁面。" border="false":::

1. 在側邊面板中，選取 [ **兌換產品金鑰**]。 

1. 輸入產品金鑰，然後選取 [ **驗證** ] 以驗證金鑰詳細資料。 

1. 選取 **兌換** 以完成此程式。

   > [!NOTE]
   > 每個產品金鑰可為使用者提供12個月的服務，從您兌換產品金鑰的日期起算。 但是，如果您現有的使用者有提早的續約日期，則會將兩個日期平均，讓您能夠更輕鬆地根據所有使用者的單一結束日期來管理您的訂閱。 在計算期間，不會新增或遺失任何時間。 用來計算兩個日期的平均值如下：
   >
   > **計算：** \[ (初始使用者) x B (訂用帳戶上剩餘的時間) \+ C (新使用者) x (12 個月，) \] ÷使用者總數 = 新的剩餘時間。

## <a name="activate-a-subscription-for-the-first-time"></a>第一次啟動訂用帳戶

如果您是第一次啟用訂用帳戶，請遵循下列步驟。 如果您的訂用帳戶處於停用或已過期狀態，您也可以使用下列步驟：

1. 複製下列連結，並將其貼入 **私用或 incognito 瀏覽器視窗**。
   
   [https://signup.microsoft.com/productkeystart](https://signup.microsoft.com/productkeystart)
1. 請確定您已登入正確的 Azure Active Directory 租使用者帳戶。 登入後，請確認租使用者詳細資料。

   :::image type="content" source="images/benefits/mpn-benefits-azure-cloud-5.png" alt-text="顯示要在哪裡新增金鑰的 Azure 和雲端產品頁面。" border="false":::

1. 輸入從合作夥伴中心取得的25位數金鑰，然後選取 **[下一步]**。

2. 確認顯示的產品詳細資料、授權數目和訂用帳戶持續時間。 當您準備好時，請選取 **兌換** 來啟用訂用帳戶。

   新啟用的訂用帳戶將會出現在 M365 系統管理中心的 [ **產品** ] 索引標籤中。

## <a name="licensing-scenarios"></a>授權案例

本節說明與 Azure 和雲端產品相關聯之訂用帳戶的一些範例授權案例。 在這些案例中，您將瞭解如何計算訂用帳戶的結束日期。

### <a name="scenario-1"></a>案例 1

如果您在訂用帳戶上有使用中的100個授權，而您嘗試使用具有25個授權的金鑰來延長結束日期，則會將結束日期延長為具有100授權之訂用帳戶的三個 (3) 個月。

### <a name="scenario-2"></a>案例 2

如果您在訂用帳戶上有使用中的100個授權，而您嘗試使用具有100授權的金鑰來延長結束日期，則結束日期將會延長12個月。

### <a name="scenario-3"></a>案例 3

如果您的訂用帳戶有五個 (5 個) 授權，而您嘗試使用具有10個授權的金鑰來延長結束日期，則會為訂用帳戶延長為期24個月的時間，而訂用帳戶有五個 (5) 授權 (或基座) 。

### <a name="scenario-4"></a>案例 4

如果您嘗試使用具有3倍以上作用中授權/基座的金鑰來延長結束日期，則會出現下列錯誤訊息。 這是設計的訊息，因為訂閱結束日期最多可延伸3年。
