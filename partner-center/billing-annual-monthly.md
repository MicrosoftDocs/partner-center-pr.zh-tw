...
本文說明合作夥伴中心的**每月帳單**和**年度計費**之間的差異，包括優點和使用案例。 您可以選擇每月或每年支付特定雲端解決方案提供者（CSP）訂用帳戶的費用。

## <a name="applicability"></a>適用性

大部分以授權為基礎的訂用帳戶都可以選擇 [每月] 或 [年度計費] 選項。 用量型訂閱只有每月計費選項。

**每個訂**用帳戶的年度和每月計費均為， **_而不_是每個授權**。
...
如果您想要調整計費日期以符合個別客戶的日期，則從年度計費切換為每月計費可能會很有用。
...
對於具有相同供應專案之**多個**訂用帳戶的客戶，每個訂用帳戶可能會有不同的計費頻率。 有一些供應專案僅限每個客戶有一個訂用帳戶。 若供應項目並沒有進行限制，則客戶就可以以不同的帳單週期，針對同一項供應項目購買多個訂閱。 您可以在方案組合的 I 欄中找到所有供應項目的限制。 您可以在合作夥伴中心的查看供應專案**和定價**一節中找到供應專案矩陣。

### <a name="free-subscription-period"></a>免費訂閱期間

年度計費頻率的訂閱不會收到免費的期間。 12個月的付費詞彙開始于購買日期。 這不同于訂用帳戶，每月計費頻率會從購買日期到下一個計費日期的免費期間接收。
...
針對年度計費，如果在12個月付的前30天內取消訂用帳戶，您將會在下一個計費日期收到100% 的信用額度。 如果在12個月期的30天后取消訂用帳戶，您將會在下一個計費日期收到按比例計算的信用額度。

### <a name="moving-subscriptions-between-partners"></a>在合作夥伴之間移動訂閱

客戶無法將訂用帳戶之間的訂閱移至另一個夥伴。 這項限制適用于每月和每年計費的訂閱。
...
如果供應專案價格在計費期間降低，則您的計費金額不會變更。 價格會針對購買時的完整計費週期進行設定。 此慣例適用于每月和每年計費。
...
- 更新訂用帳戶時，將會顯示對帳檔案的下一次變更。 此變更將會在更新後的第一個計費日期出現。
...
當您將授權新增至訂用帳戶時，對帳檔案會包含點數和按比例計算的 rebill。 此慣例適用于每月和每年計費的訂閱。
